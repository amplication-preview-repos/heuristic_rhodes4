/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { YouTubeChannelWhereUniqueInput } from "./YouTubeChannelWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { YouTubeChannelUpdateInput } from "./YouTubeChannelUpdateInput";

@ArgsType()
class UpdateYouTubeChannelArgs {
  @ApiProperty({
    required: true,
    type: () => YouTubeChannelWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => YouTubeChannelWhereUniqueInput)
  @Field(() => YouTubeChannelWhereUniqueInput, { nullable: false })
  where!: YouTubeChannelWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => YouTubeChannelUpdateInput,
  })
  @ValidateNested()
  @Type(() => YouTubeChannelUpdateInput)
  @Field(() => YouTubeChannelUpdateInput, { nullable: false })
  data!: YouTubeChannelUpdateInput;
}

export { UpdateYouTubeChannelArgs as UpdateYouTubeChannelArgs };
